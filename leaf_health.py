...
def predict(image):
    ...
    label_to_probabilities = sorted(label_to_probabilities, key=lambda element: element[1])

    result = { 'healthy' : 0 , 'diseased' : 0 }
    
    result = f"{label_to_probabilities[np.argmax(probabilities)][0]} with a { (100 * np.max(probabilities)).round(2)} % confidence." 
